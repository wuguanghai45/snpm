...
// every time we dive into a deeper spm_modules folder, the "family" list that
    ...
    if (!args.length) {
      ...
      if (npm.config.get("global")) args = ["."]
        ...
        if (data.name === path.basename(where) &&
            path.basename(path.dirname(where)) === "spm_modules") {
...
// where/spm_modules to the family object.
...
function installManyTop_ (what, where, context, cb) {
  var nm = path.resolve(where, "spm_modules")
  ...
  fs.readdir(nm, function (er, pkgs) {
    ...
    maybeScoped(scopes, nm, function (er, scoped) {
      ...
      asyncMap(unscoped.concat(scoped).map(function (p) {
        ...
        // if there's nothing in spm_modules, then don't freak out.
        if (er) packages = []
...
function targetResolver (where, context, deps, devDeps) {
  var alreadyInstalledManually = []
    , resolveLeft = 0
    , nm = path.resolve(where, "spm_modules")
...
function installOne_ (target, where, context, cb_) {
  ...
  function changePkg(dest) {
    var pkgPath = path.join(dest, 'package.json');
    // https://github.com/spmjs/spm/issues/1183
    var pkg = fs.existsSync(pkgPath) ? readJSON(pkgPath) : {};

    pkg.spm = pkg.spm || {};
    if (pkg.main && pkg.main.split(".")[pkg.main.split(".").length - 1] !== "js")  {
      pkg.spm["main"] = pkg.main + ".js";
    } else {
      pkg.spm["main"] = pkg.main;
    }
    pkg.spm["dependencies"] = pkg.dependencies;
  ...
  chain(
    [ [ cache.unpack, target.name, target.version, targetFolder, null, null, user, group ],
      ...
      function collectBundled (cb) {
        ...
        var bd = path.resolve(targetFolder, "spm_modules")
